...
bundleiq/agent.py
-----------------
Builds and runs the BundleIQ LangGraph agent.

Session 8: tools.py now uses MultiServerMCPClient to discover query_plans and
query_promotions from the Session 7 MCP server. Graph structure is unchanged
from Session 5.

Run with:
    python -m bundleiq.agent   (from inside s08/solution/)
"""
import os
import sqlite3
...
def run() -> None:
    ...
    print("  Tools: MCP server (query_plans, query_promotions)")
    print("  Type 'quit' to exit")
    print("=" * 55)
    print(f"  Session: {thread_id[:8]}...")
    if os.getenv("LANGSMITH_TRACING", "").lower() == "true":
        project = os.getenv("LANGSMITH_PROJECT", "batch1-bundleiq")
