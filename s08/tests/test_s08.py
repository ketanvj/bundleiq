...
Importing bundleiq.tools connects to the real Session 7 MCP server once,
...
from bundleiq.config import MCP_SERVER_PATH   # noqa: E402
...
from bundleiq.tools import _extract_text, _run_tool   # noqa: E402
from bundleiq.nodes import classify, decline, escalate  # noqa: E402
...
class TestMCPToolLoading:
    ...
    def test_mcp_tools_contains_query_plans(self):
        names = [t.name for t in _tools.mcp_tools]
        assert "query_plans" in names

    def test_mcp_tools_contains_query_promotions(self):
        names = [t.name for t in _tools.mcp_tools]
        assert "query_promotions" in names

    def test_tool_registry_maps_names_to_tools(self):
        assert set(_tools._tool_registry.keys()) == {"query_plans", "query_promotions"}
...
class TestExtractText:
    def test_single_text_block(self):
        result = _extract_text([{"type": "text", "text": "Daily 1GB: Rs. 179"}])
        assert result == "Daily 1GB: Rs. 179"
...
class TestRunTool:
    ...
    def test_dispatches_query_plans(self):
        fake = self._fake_tool(return_value=[{"type": "text", "text": "Daily 1GB: Rs. 179"}])
        with patch.dict(_tools._tool_registry, {"query_plans": fake}):
            result = _run_tool("query_plans", {"plan_type": "mobile"})
        assert result == "Daily 1GB: Rs. 179"
        fake.ainvoke.assert_called_once_with({"plan_type": "mobile"})

    def test_dispatches_query_promotions(self):
        fake = self._fake_tool(return_value=[{"type": "text", "text": "Monsoon Data Offer"}])
        with patch.dict(_tools._tool_registry, {"query_promotions": fake}):
            result = _run_tool("query_promotions", {"plan_id": "mob_001"})
        assert result == "Monsoon Data Offer"
    ...
    def test_returns_string(self):
        ...
        with patch.dict(_tools._tool_registry, {"query_plans": fake}):
            result = _run_tool("query_plans", {"plan_type": "all"})
...
class TestGraphNodes:
    def _make_state(self, message="test", query_type="SIMPLE") -> BundleIQState:
        return {
...
class TestBuildGraph:
    ...
    def test_graph_invoke_complex_returns_escalation(self):
        ...
        with patch.object(_nodes, "classifier_llm") as mock_llm:
            ...
            graph = build_graph(checkpointer=MemorySaver())
            result = graph.invoke(
                {"customer_message": "Which plan is best for my family?", "response": ""},
