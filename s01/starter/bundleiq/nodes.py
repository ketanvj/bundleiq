...
bundleiq/nodes.py
-----------------
Node functions for the BundleIQ graph.

Each node is a plain Python function:
  - Input : the full BundleIQState (read-only)
...
#      On exception → print the error with a [BundleIQ] prefix
...
def respond(state: BundleIQState) -> dict:
    ...
    raise NotImplementedError("TODO 4: implement respond() in bundleiq/nodes.py")
