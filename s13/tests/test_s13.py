...
---------------------
Tests for Session 13: Streamlit UI + Human-in-the-Loop (BundleIQ).
...
class TestBuildInputState:
    def test_has_customer_message(self):
        state = build_input_state("What plan suits me?")
        assert state["customer_message"] == "What plan suits me?"
    ...
    def test_different_messages_differ(self):
        assert build_input_state("plan A")["customer_message"] != build_input_state("plan B")["customer_message"]
...
class TestFormatRouteLabel:
    def test_includes_query_type(self):
        result = {"query_type": "PLANS", "specialist": "plans_agent", "compliance_status": "PASS"}
        assert "PLANS" in format_route_label(result)

    def test_includes_specialist(self):
        result = {"query_type": "PLANS", "specialist": "plans_agent", "compliance_status": "PASS"}
        assert "plans_agent" in format_route_label(result)
    ...
    def test_revised_badge_shown(self):
        result = {"query_type": "PLANS", "specialist": "plans_agent", "compliance_status": "REVISED"}
...
class TestAgentGraph:
    ...
    def test_graph_invocable_returns_response(self):
        ...
        with patch.object(_nodes, "classifier_llm") as mock_clf, \
            ...
            mock_pa.invoke.return_value  = {"response": "Your plan costs Rs. 499.", "history": []}
            mock_ca.invoke.return_value  = {"response": "Your plan costs Rs. 499.", "compliance_status": "PASS"}
            graph  = build_graph(checkpointer=MemorySaver())
            result = graph.invoke(
                build_input_state("What plan am I on?"),
    ...
    def test_revised_result_triggers_hitl(self):
        ...
        with patch.object(_nodes, "classifier_llm") as mock_clf, \
             ...
             patch.object(_nodes, "_compliance_agent") as mock_ca:
            mock_clf.invoke.return_value = MagicMock(content="PLANS")
            mock_pa.invoke.return_value  = {"response": "Guaranteed best plan!", "history": []}
            mock_ca.invoke.return_value  = {
                "response":          "Here are our available plans.",
                "compliance_status": "REVISED",
            }
            graph  = build_graph(checkpointer=MemorySaver())
            result = graph.invoke(
                build_input_state("What is the best plan?"),
