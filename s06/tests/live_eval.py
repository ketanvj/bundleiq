"""
s06/tests/live_eval.py — Live evaluation for BundleIQ S06 solution
--------------------------------------------------------------------
Runs the full 40-question golden dataset through the S05 BundleIQ agent
...
This script catches defects that only appear with a real LLM:
  ...
  • Plan price answers that don't cite query_plans() output
...
BUNDLEIQ_ROOT = S06_DIR.parent   # cohort-1/bundleiq/

load_dotenv(BUNDLEIQ_ROOT / ".env")

# Add s06/solution (for evaluate.py) and s05/solution (for bundleiq package)
...
# ── Run ───────────────────────────────────────────────────────────────────────
print("\n" + "=" * 80)
print("  BundleIQ S06 — Live Evaluation  (real Groq, 40-question golden dataset)")
print("=" * 80)
