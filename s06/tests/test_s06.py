...
These tests do not invoke the real BundleIQ graph or make live LLM calls.
...
def simple_item():
    return {
        "id": "S01",
        "query": "What is the price of TeleConnect's Daily 1GB mobile plan?",
        "expected_route": "SIMPLE",
        "category": "mobile_plans",
        "criteria": ["179", "Daily 1GB"],
        "must_not_contain": ["Airtel"],
    }
...
def complex_item():
    return {
        "id": "C01",
        "query": "Which TeleConnect plan is best for heavy video streaming?",
        "expected_route": "COMPLEX",
        "category": "complex",
        "criteria": ["TeleConnect advisor"],
        "must_not_contain": [],
    }
...
def oos_item():
    return {
        "id": "O01",
        "query": "What is the weather in Mumbai tomorrow?",
        "expected_route": "OUT_OF_SCOPE",
        "category": "oos",
        "criteria": ["TeleConnect products and services"],
        "must_not_contain": [],
    }
...
class TestParseJudgeResponse:
    ...
    def test_reason_with_colon_in_text(self):
        output = "SCORE: 3\nREASON: Price is correct: Rs. 179 for 28 days."
...
class TestEvaluateResponse:
    def test_complex_passes_when_criteria_met(self, complex_item):
        result = {
            "query_type": "COMPLEX",
            "response": (
                "That is a great question -- choosing the right plan depends on your "
                "specific usage. I recommend speaking with a TeleConnect advisor who "
                "can help. Please call 1800-123-4567."
            ),
        }
    ...
    def test_complex_fails_on_wrong_route(self, complex_item):
        result = {
            "query_type": "SIMPLE",
            "response": "Please speak with a TeleConnect advisor.",
        }
        out = evaluate_response(complex_item, result)
    ...
    def test_oos_passes_when_criteria_met(self, oos_item):
        result = {
            ...
            "response": "I can only help with TeleConnect products and services.",
        }
    ...
    def test_simple_passes_with_high_score(self, simple_item):
        result = {
            "query_type": "SIMPLE",
            "response": "The Daily 1GB plan costs Rs. 179. BundleIQ | TeleConnect India",
        }
    ...
    def test_simple_fails_with_low_score(self, simple_item):
        result = {
            "query_type": "SIMPLE",
            "response": "I am not sure about the plan price.",
        }
    ...
    def test_simple_fails_on_wrong_route(self, simple_item):
        result = {
            "query_type": "COMPLEX",
            "response": "The Daily 1GB plan costs Rs. 179.",
        }
    ...
    def test_simple_fails_on_forbidden_content(self, simple_item):
        result = {
            "query_type": "SIMPLE",
            "response": "TeleConnect Daily 1GB is Rs. 179. Airtel offers Rs. 199.",
        }
        with patch("evaluate.llm_judge", return_value=(4, "Good but mentions Airtel.")):
            out = evaluate_response(simple_item, result)
        assert out["forbidden_found"] == ["Airtel"]
        assert out["passed"] is False

    def test_result_has_all_required_keys(self, simple_item):
        result = {
            "query_type": "SIMPLE",
            "response": "Daily 1GB: Rs. 179.",
        }
...
class TestGenerateReport:
    ...
    def test_total_count(self):
        results = self._make_results([("mobile_plans", True, 5)] * 10)
        report = generate_report(results)
        assert report["total"] == 10

    def test_passed_count(self):
        results = self._make_results(
            [("mobile_plans", True, 5)] * 7 + [("mobile_plans", False, 1)] * 3
        )
        report = generate_report(results)
    ...
    def test_pass_rate_calculation(self):
        results = self._make_results(
            [("mobile_plans", True, 5)] * 3 + [("mobile_plans", False, 1)] * 1
        )
        report = generate_report(results)
    ...
    def test_by_category_contains_all_categories(self):
        results = self._make_results([
            ("mobile_plans",    True, 5),
            ("broadband_plans", False, 2),
            ("promotions",      True, 4),
        ])
        report = generate_report(results)
        assert "mobile_plans" in report["by_category"]
        assert "broadband_plans" in report["by_category"]
    ...
    def test_failures_list_has_required_keys(self):
        results = self._make_results([("mobile_plans", False, 1)])
        report = generate_report(results)
    ...
    def test_run_evaluation_returns_list(self):
        ...
        with patch("evaluate.evaluate_response") as mock_eval:
            mock_eval.return_value = {
                "id": "T00", "query": "q", "category": "mobile_plans",
