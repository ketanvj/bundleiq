"""
BundleIQ -- Session 6: Baseline Evaluation (US-05)
===================================================

What you build this session
  A structured evaluation pipeline that runs 40 questions from a golden
  dataset through the Session 5 BundleIQ agent, scores each response,
...
Run
  ...
  The script imports the built BundleIQ graph from s05/solution/bundleiq/agent.py
...
Golden dataset
  s06/data/golden_dataset.json  --  40 Q&A items:
    20 SIMPLE  (mobile plans, broadband plans, promotions, plan features)
    10 COMPLEX (comparison, recommendation)
...
JUDGE_PROMPT = """You are evaluating a telecom AI assistant's response to a customer question.
...
def parse_judge_response(output: str) -> tuple[int, str]:
    ...
    Expected format:
      SCORE: 4
      REASON: The response correctly states the plan price and validity.
...
def print_report(report: dict) -> None:
    ...
    print("\n" + "=" * 60)
    print("  BundleIQ Baseline Evaluation Report")
    print("=" * 60)
...
def main() -> None:
    ...
    # Import the BundleIQ graph from Session 5
