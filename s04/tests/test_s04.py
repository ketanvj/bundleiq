...
Run from the bundleiq/ directory:
    pytest s04/tests/ -v
...
from bundleiq.config import DECLINE_RESPONSE, ESCALATE_RESPONSE, RETRIEVAL_K, SYSTEM_PROMPT  # noqa: E402
from bundleiq.state import BundleIQState  # noqa: E402
import bundleiq.nodes as _nodes  # noqa: E402
from bundleiq.nodes import classify, decline, escalate, respond, retrieve_docs, route_query  # noqa: E402
...
def _make_mock_doc(page_content: str, source: str = "plans_guide.md") -> MagicMock:
    doc = MagicMock()
...
def mock_llm_simple():
    with patch.object(_nodes, "llm") as mock_main, \
        ...
        mock_main.invoke.return_value = MagicMock(
            content="The Classic plan includes 2GB/day data. BundleIQ | TeleConnect India"
        )
...
def mock_vectorstore():
    doc = _make_mock_doc(
        "The Classic plan offers 2GB of daily data with unlimited calls and SMS, "
        "valid for 28 days. Speed reduces to 512 kbps after daily limit.",
        source="plans_guide.md",
    )
...
class TestBundleIQState:
    ...
    def test_state_has_retrieved_docs(self):
        assert "retrieved_docs" in BundleIQState.__annotations__, (
            "BundleIQState must have a 'retrieved_docs' field (added in Session 4). "
            "Add it after 'query_type' with type hint list[str]."
        )

    def test_retrieved_docs_is_list_type(self):
        annotation = BundleIQState.__annotations__["retrieved_docs"]
        origin = getattr(annotation, "__origin__", annotation)
        assert origin is list

    def test_state_instantiable_with_all_fields(self):
        state: BundleIQState = {
            "customer_message": "What data does the Classic plan include?",
            "response":         "",
            "history":          [],
            "query_type":       "SIMPLE",
            "retrieved_docs":   [],
    ...
    def test_state_retrieved_docs_accepts_strings(self):
        state: BundleIQState = {
            ...
            "history":          [],
            "query_type":       "SIMPLE",
            "retrieved_docs":   ["[plans_guide.md]\nClassic plan: 2GB/day."],
        }
...
class TestRetrieveDocsNode:
    def _state(self, question: str = "What does the Classic plan include?") -> BundleIQState:
        return {
    ...
    def test_retrieve_docs_calls_similarity_search(self, mock_vectorstore):
        retrieve_docs(self._state("What is the Fair Usage Policy?"))
    ...
    def test_retrieve_docs_passes_question_to_search(self, mock_vectorstore):
        question = "What does the Classic plan include?"
...
class TestRespondWithContext:
    def _state_with_docs(self, docs: list[str]) -> BundleIQState:
        return {
            "customer_message": "What does the Classic plan include?",
            "response":         "",
            "history":          [],
            "query_type":       "SIMPLE",
            "retrieved_docs":   docs,
    ...
    def test_respond_includes_retrieved_docs_in_system_message(self):
        chunk = "[plans_guide.md]\nClassic plan: 2GB/day, unlimited calls."
        state = self._state_with_docs([chunk])
        with patch.object(_nodes, "llm") as mock_llm:
            mock_llm.invoke.return_value = MagicMock(content="Classic includes 2GB/day.")
            respond(state)
        call_args   = mock_llm.invoke.call_args
        messages    = call_args[0][0]
        system_text = messages[0].content
        assert "2GB" in system_text or "plans_guide.md" in system_text
    ...
    def test_respond_context_contains_policy_keyword(self):
        chunk = "[broadband_guide.md]\nInstallation requires technician visit within 3 working days."
        state = self._state_with_docs([chunk])
        with patch.object(_nodes, "llm") as mock_llm:
            mock_llm.invoke.return_value = MagicMock(content="Installation takes 3 working days.")
            respond(state)
        call_args   = mock_llm.invoke.call_args
        messages    = call_args[0][0]
        system_text = messages[0].content
    ...
    def test_respond_with_multiple_docs(self):
        chunks = [
            "[plans_guide.md]\nClassic plan includes 2GB daily data.",
            "[faq.md]\nData balance resets at midnight every day.",
        ]
        ...
        messages    = call_args[0][0]
        system_text = messages[0].content
        assert "2GB" in system_text or "Classic" in system_text
        assert "midnight" in system_text or "resets" in system_text

    def test_respond_updates_history_with_docs(self):
        chunk = "[faq.md]\nYou can check your balance by sending BAL to 198."
        state = self._state_with_docs([chunk])
        with patch.object(_nodes, "llm") as mock_llm:
            mock_llm.invoke.return_value = MagicMock(content="Send BAL to 198.")
            result = respond(state)
...
class TestGraphRouting:
    def test_simple_path_calls_vectorstore(self, mock_vectorstore, mock_llm_simple, memory_checkpointer):
        ...
        graph.invoke(
            {"customer_message": "What does the Classic plan include?", "response": ""},
            config=config,
        )
        mock_vectorstore.similarity_search.assert_called_once()
    ...
    def test_all_paths_produce_non_empty_response(self, mock_vectorstore, memory_checkpointer):
        for query_type, question in [
            ("SIMPLE",       "What does the Classic plan include?"),
            ("COMPLEX",      "Which plan should I choose?"),
...
class TestMemoryWithRAG:
    def test_history_accumulates_across_simple_turns(self, mock_vectorstore, mock_llm_simple, memory_checkpointer):
        ...
        result = graph.invoke(
            {"customer_message": "Does Premium have OTT packs?", "response": ""},
            config=config,
        )
    ...
    def test_different_threads_isolated(self, mock_vectorstore, mock_llm_simple, memory_checkpointer):
        ...
        graph.invoke(
            {"customer_message": "What is the Unlimited plan price?", "response": ""},
