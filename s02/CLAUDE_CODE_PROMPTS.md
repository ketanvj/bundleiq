...
**BundleIQ | Multi-Turn Memory**

---

## How to use this sheet

These are prompts you type into Claude Code to add conversation memory to BundleIQ.
...
Open `s02/bundleiq/` alongside this sheet.
There are 4 TODOs spread across 4 files. Fill them in order 1 → 4.

**What you are building:**
After this session, a customer can say "I need international roaming" in one
turn and ask "which plan includes it?" in the next — and BundleIQ will remember
the context without being told again.

---

## TODO 1 — Call load_dotenv() `bundleiq/__init__.py`

Same as Session 1. The `__init__.py` is the first file Python runs when
the `bundleiq` package loads — so the API key must be available here
before `config.py` or `tools.py` try to read it.

```
In bundleiq/__init__.py, fill in the two steps at the TODO 1 block:
...
## TODO 2 — Add the history field to BundleIQState `bundleiq/state.py`
...
```
In bundleiq/state.py, add one field to BundleIQState below the
...
## TODO 3 — Update respond() to use history `bundleiq/nodes.py`
...
```
In bundleiq/nodes.py, replace the TODO placeholder in respond() with
...
## TODO 4 — Wire the checkpointer in build_graph() `bundleiq/agent.py`

**What it does:** attaches a SQLite checkpointer so LangGraph saves state
between turns — even if the script restarts.

```
In bundleiq/agent.py, uncomment the build_graph() function and delete
the two placeholder lines below it.

The function should:
1. Create the builder:   builder = StateGraph(BundleIQState)
...
Look at my respond() in bundleiq/nodes.py and check that the loop
...
Look at my build_graph() in bundleiq/agent.py and make sure I call
...
but NOT with a "history" key, even though history is part of BundleIQState.
```
...
**Show the conversation length:**
```
In bundleiq/agent.py, after printing the BundleIQ response, also print
...
```
In bundleiq/nodes.py, before building the message list, trim history
