...
data/ingest.py
--------------
Ingests TeleConnect India documents into ChromaDB.
...
Why are prices NOT in these documents?
  Prices are in data/teleconnect_data.db (see seed.py). Putting "Daily 2GB plan:
  Rs. 299" in a document would create two sources of truth. When prices change
  (seed.py is re-run), the document would still show the old figure. The compliance
  node checks that BundleIQ never quotes prices from documents -- prices must
  always come from a tool call to this database.
...
def load_documents() -> List[Document]:
    ...
    if not DOCS_DIR.exists():
        ...
        print("Make sure you are running this script from the bundleiq/ folder.", file=sys.stderr)
        sys.exit(1)
...
def main() -> None:
    print("Ingesting TeleConnect India documents into ChromaDB")
    ...
    print("\nSetup complete. Run 'python s01/solution/main.py' to start BundleIQ.")
