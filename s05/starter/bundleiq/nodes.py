...
bundleiq/nodes.py
-----------------
Graph nodes and routing for BundleIQ.

Session 5: respond() gains tool-calling -- it calls query_plans and/or
query_promotions when the LLM requests them, then makes a second LLM call
...
def respond(state: BundleIQState) -> dict:
    ...
    try:
        ...
        #           print(f"[BundleIQ] Tool: {tc['name']}({tc['args']}) -> {str(tool_output)[:80]}")
        #           messages.append(
