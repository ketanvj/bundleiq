...
-----------------------
Tests for Session 14b: Security and Guardrails with LlamaGuard 3 (BundleIQ).
...
class TestInjectionPatterns:
    ...
    def test_normal_plan_query_not_matched(self):
        assert not self._matches("What are the 5G mobile plans?")

    def test_promotions_query_not_matched(self):
        assert not self._matches("Are there any cashback offers right now?")

    def test_greeting_not_matched(self):
        assert not self._matches("Hello, I need help with my broadband plan")
...
class TestPiiPatterns:
    ...
    def test_normal_message_with_no_pii(self):
        assert not self._matches("What is the 5G plan price?")
    ...
    def test_short_number_not_matched(self):
        assert not self._matches("I want the 299 plan")
...
class TestGuardFunction:
    ...
    def test_clean_message_returns_empty_blocked_reason(self):
        result = guard(self._make_state("What are the 5G mobile plans?"))
    ...
    def test_promotions_query_clean(self):
        result = guard(self._make_state("What cashback offers are available?"))
        assert result["blocked_reason"] == ""
...
class TestLlamaGuardLayer:
    """_llamaguard_safe() parses LlamaGuard 3 8B 'safe'/'unsafe\\nS<n>' correctly."""
    ...
    def test_llamaguard_safe_returns_true_for_safe_verdict(self):
        with patch.object(_nodes, "llamaguard_llm") as mock_llm:
            ...
            assert _nodes._llamaguard_safe("What are the 5G plans?") is True
    ...
    def test_llamaguard_s6_alone_passes_through(self):
        # S6 (Specialized Advice) is handled by COMPLEX → escalate.
        # LlamaGuard S6-only findings must NOT block — the router handles them.
        with patch.object(_nodes, "llamaguard_llm") as mock_llm:
            mock_llm.invoke.return_value = MagicMock(content="unsafe\nS6")
            assert _nodes._llamaguard_safe("Which plan is best for me?") is True
    ...
    def test_llamaguard_s6_combined_with_other_category_blocks(self):
        ...
        with patch.object(_nodes, "llamaguard_llm") as mock_llm:
            mock_llm.invoke.return_value = MagicMock(content="unsafe\nS6,S13")
            assert _nodes._llamaguard_safe("jailbreak with advice request") is False
...
class TestAgentGraph:
    ...
    def test_clean_query_reaches_classifier(self):
        from langgraph.checkpoint.memory import MemorySaver
        with patch.object(_nodes, "classifier_llm") as mock_clf, \
             patch.object(_nodes, "_plans_agent") as mock_pa, \
             patch.object(_nodes, "_compliance_agent") as mock_ca, \
             patch.object(_nodes, "_llamaguard_safe", return_value=True):
            mock_clf.invoke.return_value = MagicMock(content="PLANS")
            mock_pa.invoke.return_value  = {
                "response":   "The Starter 5G plan is Rs. 399/month.",
                "history":    [],
            ...
            graph  = build_graph(checkpointer=MemorySaver())
            result = graph.invoke(
                build_input_state("What are the 5G plans?"),
...
class TestAppHelpers:
    ...
    def test_build_input_state_has_no_llamaguard_score(self):
        # S14b state has no llamaguard_score field (LlamaGuard 3 returns category codes, not floats)
        state = build_input_state("test")
    ...
    def test_format_route_label_normal_query(self):
        result = {"blocked_reason": "", "specialist": "plans_agent", "compliance_status": "PASS"}
        label  = format_route_label(result)
        assert "plans_agent" in label
